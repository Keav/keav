...
if( isset($HTTP_POST_VARS['doprune']) )
{
   ...
   $template->assign_vars(array(
      ...
      'L_FORUM' => isset($cat_id) ? $lang['Category'] : $lang['Forum'],
      'L_TOPICS_PRUNED' => $lang['Topics_pruned'],
      'L_POSTS_PRUNED' => $lang['Posts_pruned'],
      'L_PRUNE_RESULT' => $message)
   );
}
else
{
   ...
   //
   if( empty($HTTP_POST_VARS[POST_FORUM_URL]) && empty($HTTP_GET_VARS[POST_FORUM_URL]) && empty($HTTP_POST_VARS[POST_CAT_URL]) && empty($HTTP_GET_VARS[POST_CAT_URL]) )
   {
      //
   ...
   else
   {
      ...
      $forum_name = ( ( isset($cat_id) ? $cat_id : $forum_id ) == -1 ) ? $lang['All_Forums'] : ( isset($cat_id) ? $forum_rows[0]['cat_title'] : $forum_rows[0]['forum_name'] );

      $prune_data = $lang['Prune_topics_not_posted'] . " "; 
      $prune_data .= '<input class="post" type="text" name="prunedays" size="4"> ' . $lang['Days'];

      $hidden_input = '<input type="hidden" name="' . ( isset($cat_id) ? POST_CAT_URL : POST_FORUM_URL ) . '" value="' . $forum_id . '" />';
